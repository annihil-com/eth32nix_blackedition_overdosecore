...
void CGui::Init(void)
{
	...
	if (init) {
		...
		ChatMessage("^7ETH^332nix ^7(^3c^7)2009 - nixcoders.org");
		return;
	}
	...
	winPicker->AddWindow("Main", CAT_AIMBOT);
	winPicker->AddWindow("Weapon Config", CAT_HITBOX);
	winPicker->AddWindow("Aim Prediction", CAT_AIMBOT);
	winPicker->AddWindow("Humanaim", CAT_HUMAN);
	winPicker->AddWindow("Custom Hitbox", CAT_HITBOX);
	winPicker->AddWindow("Hitbox & Trace", CAT_HITBOX);
	winPicker->AddWindow("Corrections", CAT_HITBOX);
	...
	winPicker->AddWindow("Hitbox Display", CAT_HITBOX);
